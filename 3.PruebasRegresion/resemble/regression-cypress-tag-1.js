...
async function executeTest(){
    ...
    for(var i = 1; i<= nroStep; i++){
        var folder = `cypress-tag-1-${datetime}`;
        if (!fs.existsSync(`./results/${folder}`)){
            fs.mkdirSync(`./results/${folder}`, { recursive: true });
        }
        
        const data = await compareImages(
            fs.readFileSync(`../Ghost34/PruebasCypress/cypress/screenshots/Tags/createTagCypress1.spec.js/Tests case creación de tags -- Crear Tag (${i}).png`),
            fs.readFileSync(`../Ghost44/PruebasCypress/cypress/screenshots/Tags/createTagCypress1.spec.js/Tests case creación de tags -- Crear Tag (${i}).png`),
            options
        );
        resultInfo[i] = {
