...
async function executeTest(){
    ...
    for(var i = 1; i<= nroStep; i++){
        var folder = `cypress-tag-2-${datetime}`;
        if (!fs.existsSync(`./results/${folder}`)){
            fs.mkdirSync(`./results/${folder}`, { recursive: true });
        }
        
        const data = await compareImages(
            fs.readFileSync(`../Ghost34/PruebasCypress/cypress/screenshots/Tags/createTagCypress5.spec.js/Tests case creación de tags -- Crear Tag with wrong color (${i}).png`),
            fs.readFileSync(`../Ghost44/PruebasCypress/cypress/screenshots/Tags/createTagCypress5.spec.js/Tests case creación de tags -- Crear Tag with wrong color (${i}).png`),
            options
        );
        resultInfo[i] = {
