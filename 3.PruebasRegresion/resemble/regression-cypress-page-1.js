...
async function executeTest(){
    ...
    for(var i = 1; i<= nroStep; i++){
        var folder = `cypress-page-1-${datetime}`;
        if (!fs.existsSync(`./results/${folder}`)){
            fs.mkdirSync(`./results/${folder}`, { recursive: true });
        }
        
        const data = await compareImages(
            fs.readFileSync(`../Ghost34/PruebasCypress/cypress/screenshots/Page/editPagesCypress1.spec.js/Tests edición de Pages -- Editar una pagina despues de creada (${i}).png`),
            fs.readFileSync(`../Ghost44/PruebasCypress/cypress/screenshots/Page/editPagesCypress1.spec.js/Tests edición de Pages -- Editar una pagina despues de creada (${i}).png`),
            options
        );
        resultInfo[i] = {
