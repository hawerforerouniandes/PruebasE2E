...
async function executeTest(){
    ...
    for(var i = 1; i<= nroStep; i++){
        var folder = `cypress-page-3-${datetime}`;
        if (!fs.existsSync(`./results/${folder}`)){
            fs.mkdirSync(`./results/${folder}`, { recursive: true });
        }
        
        const data = await compareImages(
            fs.readFileSync(`../Ghost34/PruebasCypress/cypress/screenshots/Page/editPagesCypress3.spec.js/Tests edición de Pages -- Editar una pagina de un draft a publicada (${i}).png`),
            fs.readFileSync(`../Ghost44/PruebasCypress/cypress/screenshots/Page/editPagesCypress3.spec.js/Tests edición de Pages -- Editar una pagina de un draft a publicada (${i}).png`),
            options
        );
        resultInfo[i] = {
