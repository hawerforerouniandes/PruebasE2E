...
async function executeTest(){
    ...
    for(var i = 1; i<= nroStep; i++){
        var folder = `post-2-${datetime}`;
        if (!fs.existsSync(`./results/${folder}`)){
            fs.mkdirSync(`./results/${folder}`, { recursive: true });
        }
        
        const data = await compareImages(
            fs.readFileSync(`../Ghost34/PruebasKraken/kraken/regression/post-2/${i}.png`),
            fs.readFileSync(`../Ghost44/PruebasKraken/kraken/regression/post-2/${i}.png`),
            options
        );
        resultInfo[i] = {
